$(function() { // this is the same as: $(document).ready(function(){
    ...
    // create a new div
    let newDiv = $('<div>'); // can also specify using '<div>Some text</div>' 

    // add some CSS, also a GET or SET method
    newDiv.css({"border":"1px solid lightgray", "padding":"10px"});

    // add some content (HTML) Equiv to innerHtml
    ...
    // Add an attribute to the cloned Div, equiv to setAttribute
    // Also an GET or set method (2-ways API)
    ...
    // Modify the value in the "exampleInputEmail1" text input, equiv eval()
    //only works on html form elements
    $("#exampleInputEmail1").val("from jQuery!");
    //Can be used to get or set. If Val() is empty, it will retrieve user's input
